...
redis.call('HSET', KEYS[1],
  ...
  'Setting', ARGV[8], 'APIBlocked', ARGV[9], 'AuthVersion', ARGV[1], 'CacheSchema', ARGV[10])
...
return 1`
	result, err := common.RDB.Eval(context.Background(), script,
		...
		user.Username, user.Setting, user.APIBlocked, user.CacheSchema, includeQuotaArg, user.Quota, ttl,
	).Int()
	if err != nil {
		return err
	}
