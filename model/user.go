...
type User struct {
	...
	APIBlocked           bool                       `json:"api_blocked" gorm:"column:api_blocked"`
	...
	ViolationCount       int                        `json:"violation_count" gorm:"type:int;default:0;"`             // moderation violations in the current window
	ViolationLimit       int                        `json:"violation_limit" gorm:"type:int;default:3;"`             // violations allowed before automatic ban
	ViolationWindowStart int64                      `json:"violation_window_start" gorm:"type:bigint;default:0;"`
...
func GetSelfUserById(id int) (*User, error) {
	...
	err := DB.Model(&User{}).Select([]string{
		...
		"stripe_customer", "auth_version", "api_blocked", "violation_count",
		"violation_limit", "violation_window_start",
...
func (user *User) prepareForInsert(tx *gorm.DB) error {
	if user.ViolationLimit <= 0 {
		user.ViolationLimit = 3
	}
