...
- “测试巡查连接”使用表单中当前填写（可以尚未保存）的地址、模型和密钥；密钥留空时服务端使用已保存的密钥或环境变量。测试仅发送固定合成文本和文件到 Responses API，不读取用户日志、不创建巡查任务或报告、不触发风险邮件，也不进入正常请求转发链路；上游调用仍可能产生少量费用。测试成功只说明所选模型对这个样本完成了文件输入请求且返回非空文本，**不能**证明自定义提示词的风控判断质量、SMTP 邮件送达或正式巡查结果。
...
- 每个**首次成功写入报告**的批次如有风险分档为“极高”或“高”的五列表格行，会通过已配置的 SMTP 向“内容审核告警邮箱”（`ModerationAlertEmail` / `MODERATION_ALERT_EMAIL`）发一封提醒，列出该批次的 `username`、行为、风险分档、简要描述和示例 request ID；邮箱留空则不发送。仅第三列精确为“极高”或“高”（允许 `<mark>` 包裹）才触发；正文其他位置出现“高风险”不会触发。邮件不会主动附上原始 JSONL 或 API Key，模型返回的字段会按纯文本转义；**模型可能在摘要中复述敏感内容**，须保护收件邮箱。SMTP 必须已配置；发送失败会写入服务日志，不影响已落盘报告或任务状态。成功标记已落盘后重跑会跳过该批次，**邮件失败也不会自动重发**，应查看服务日志并人工处理。相同内容的成功批次重跑不会重复发送。
...
- `POST /api/option/daily_review_test` 同样要求 `RootAuth`。请求 JSON 为 `{ "base_url": ".../v1", "api_key": "", "model": "..." }`；任一字段留空时服务端依次使用对应的已保存设置或默认值，API Key 优先级为表单值、`DailyReviewAPIKey`、`DAILY_REVIEW_API_KEY`、`OPENAI_API_KEY`。远端地址仍按上述 HTTPS/本机 HTTP 规则校验。服务端最多等待 30 秒，使用与正式巡查相同的 Responses 文件输入格式和 `store: false`，但提示词及附件均为固定合成样本；成功只返回状态，不回传模型生成内容。非 200 上游响应仅报告状态码，不在错误中包含可能敏感的上游响应正文。
...
| `model/system_task.go`、`service/daily_review.go`、`main.go` | `daily_review` 类型、master 定时调度、系统任务锁、内存分批、Responses 文件输入、错误追加、摘要去重、极高/高风险邮件与独立连接测试。 |
| `controller/daily_review.go`、`router/api-router.go` | Root 鉴权的手动执行、报告读取及连接测试接口、日期错误状态及管理审计。 |
...
| `web/src/features/system-settings/request-limits/moderation-section.tsx`、`web/src/features/system-settings/security/{index,section-registry}.tsx`、`web/src/features/system-settings/{api,types}.ts` | 内容审核设置区域的巡检表单、保存选项、密钥留空语义、连接测试、立即巡检和状态轮询。 |
...
冲突合并后重点检查：定时任务仍审**昨天**、手动仍审**今天**；连接测试只用合成附件、不创建任务或告警；报告按**日志日期**展示最新而非按任务创建时间；失败不会阻断后续文件；5 MB 是十进制字节上限且不切断 JSONL 行；风险邮件只针对新成功批次的极高/高风险表格行，失败不自动补发；没有把 API Key 或原文放入普通审核统计、任务审计或公开接口。不要把巡检误接到实时审核的违规计数或自动封禁链路。
