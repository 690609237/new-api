...
| 内容审核 | 支持本地敏感词检查、兼容 OpenAI 的审核 API、用户/分组豁免、采样率、强制审核令牌、缓存、告警及超时熔断。审核只针对当前请求新增的用户内容；另有读取已落盘日志的每日内容巡检，详见 [内容巡检合并参考](content-daily-review.md)。 |
...
| 首页信息区域 | 首页左侧信息卡支持在“系统管理 → 站点与品牌 → 系统信息”中使用 HTML 或 Markdown 运维；空配置时按当前界面语言显示默认内容，且不会替换首页其他区域。详细冲突处理见 [首页信息区域合并参考](home-page-content-merge-guide.md)。 |
...
本节记录业务摘要；逐文件职责、推荐合并顺序和验收清单见 [首页信息区域配置化：上线说明与合并参考](home-page-content-merge-guide.md)。
...
- “每日内容巡检”也提供独立连接测试：用表单中的地址、模型及密钥（留空时沿用服务端已保存密钥）发送合成文件输入，不读取用户日志或启动巡检，不影响正常请求。首次成功巡检批次包含极高/高风险行时，会向已配置的内容审核告警邮箱发送包含相关表格行的邮件；邮件失败不影响报告，也不会自动补发。具体配置与安全边界见 [内容巡检合并参考](content-daily-review.md)。
