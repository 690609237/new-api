...
func ManageUser(c *gin.Context) {
	...
	case "add_quota":
		...
		case "add":
			...
			recordManageAuditFor(c, user.Id, "user.quota_add", map[string]any{
		...
		case "subtract":
			...
			recordManageAuditFor(c, user.Id, "user.quota_subtract", map[string]any{
				"quota": logger.LogQuota(req.Value),
			})
		case "override":
			if err := common.ValidateWalletQuota(req.Value); err != nil {
			...
			recordManageAuditFor(c, user.Id, "user.quota_override", map[string]any{
	...
	case "set_violation_limit":
		...
		recordManageAuditFor(c, user.Id, "user.violation_limit", map[string]any{"limit": req.Value})
		c.JSON(http.StatusOK, gin.H{"success": true, "message": ""})
		return
	case "reset_violations":
		...
		recordManageAuditFor(c, user.Id, "user.violation_reset", map[string]any{})
		c.JSON(http.StatusOK, gin.H{"success": true, "message": ""})
		return
	default:
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
